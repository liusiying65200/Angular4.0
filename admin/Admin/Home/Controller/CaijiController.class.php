...
$show   = $Page->paging();
$list = $Article->field(array('id','biaoti','begtime','ssfl','bianma','url','directory4','sorting'))->where("ssfl=0")->order("sorting desc")->limit($Page->limit)->select();
...
$Page   = new  \Common\Page($count,30);
$show   = $Page->paging();
$list = $Article->field(array('id','biaoti','begtime','ssfl','bianma','url','directory4','zhuangtai','sorting'))->where("ssfl=1")->order("sorting desc")->limit($Page->limit)->select();
$Start = $Article->field(array('sorting'))->where($search)->order("sorting desc,begtime desc")->limit(1)->select();
$End   = $Article->field(array('sorting'))->where($search)->order("sorting asc,begtime desc")->limit(1)->select();
$this->assign('Startid',$Start[0]['sorting']);
$this->assign('Endid',$End[0]['sorting']);
...
$Page   = new  \Common\Page($count,30);
$show   = $Page->paging();
$list = $Article->field(array('id','biaoti','begtime','ssfl','bianma','url','directory4','zhuangtai','sorting'))->where("ssfl=2")->order("sorting desc")->limit($Page->limit)->select();
$Start = $Article->field(array('sorting'))->where($search)->order("sorting desc,begtime desc")->limit(1)->select();
$End   = $Article->field(array('sorting'))->where($search)->order("sorting asc,begtime desc")->limit(1)->select();
$this->assign('Startid',$Start[0]['sorting']);
$this->assign('Endid',$End[0]['sorting']);
...
$this->display();	
    ...
    public function kjcj(){
		...
		//采集地址
		$cjurl = "http://18kj.com/history_2016.html";
		//采集内容
        ...
        foreach ($arr as $key => $value) {
         ...
         if(D('Home/kaijiang')->where("title like '{$value[title]}' and FROM_UNIXTIME(begtime, '%Y-%m-%d') BETWEEN '2016-03-01' AND '2016-12-29'")->find()){
