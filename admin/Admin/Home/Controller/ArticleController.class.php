...
exit();	
    ...
    {
        $result=M('inner_info')->select();
        $list=[];
        foreach ($result as $v){
            $v['inner_info']=json_decode($v['inner_info'],true);
            $v['shengxiao']=json_decode($v['shengxiao'],true);
            $v['shaxiao']=json_decode($v['shaxiao'],true);
            $list[]=$v;
        }
        $this->assign('list',$list);
        $this->display();
    ...
    public function innerinfo_add()
    {
        if (IS_POST) {
            ...
            $data=I('post.');
            $data['inner_info']=json_encode($data['inner_info']);
            $data['shengxiao']=json_encode($data['shengxiao']);
            $data['shaxiao']=json_encode($data['shaxiao']);
            $id=M('inner_info')->add($data);
            if($id) {
                $this->success('保存成功');
            }else{
                $this->error('保存失败');
    ...
    public function innerinfo_edit()
    {
        if (IS_POST) {
            if(!$_POST['kithe']){
                $this->error('期数为空');
            ...
            $data['shengxiao']=json_encode($data['shengxiao']);
            $data['shaxiao']=json_encode($data['shaxiao']);
            $id=M('inner_info')->where(['id'=>$data['id']])->save($data);
            if($id) {
                $this->success('保存成功');
            }else{
                $this->error('保存失败');
            ...
            $row=M('inner_info')->find($_GET['id']);
            $row['inner_info']=json_decode($row['inner_info'],true);
            $row['shengxiao']=json_decode($row['shengxiao'],true);
            $row['shaxiao']=json_decode($row['shaxiao'],true);
            $this->assign('row',$row);
            $this->display();
        }
    }

    public function innerInfo_del()
    {
        if(!$_POST['id']) $this->error('未选择数据');
        M('inner_info')->delete($_POST['id']);
        $this->success('删除成功');
    }
    ...
    public function manage()
    {
        ...
        $count  =M('article_second')->count();
        $Page   = new  \Common\Page($count,30);
        $show   = $Page->paging();
        $list = M('article_second')->where($map)->order("edittime desc")->limit($Page->limit)->select();
        $this->assign('list',$list);
    ...
    public function manage_edit()
    {
        if (IS_POST) {
            ...
            $data=I('post.');
            $data['edittime']=$_SERVER['REQUEST_TIME'];
            $id=M('article_second')->where(['id'=>$data['id']])->save($data);
            if($id) {
                $this->success('保存成功');
            }else{
                $this->error('保存失败');
            ...
            $row=M('article_second')->find($_GET['id']);
            $this->assign('row',$row);
            $this->display();
        }
    }
    ...
    {
        if(!$_POST['id']) $this->error('未选择数据');
        M('article_second')->delete($_POST['id']);
        $this->success('删除成功');
    }
