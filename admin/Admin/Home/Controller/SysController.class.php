...
class SysController extends AuthController {
    ...
    public function runsys(){
    	if (!IS_AJAX){
    		...
    		if (!$config->autoCheckToken($_POST)){
	    		...
	    		$config->field('id,site_name,site_url,site_title,site_keywords,site_wap,site_describe')->save($sl_data);
    ...
    //底部设置
    public function sysft(){
    	$config=M('config')->where(array('id'=>1))->find();
    	$this->assign('config',$config)->display();
    }
    
    //保存底部设置
    public function runsysft(){
    	if (!IS_AJAX){
    		$this->error('提交方式不正确',0,0);
    	}else{
    		$config=M('config');
    		if (!$config->autoCheckToken($_POST)){
	    		...
	    		$config->field('id,javascript,wap_tj,site_qq,site_copyright')->save($sl_data);
	    		$this->success('站点设置保存成功',U('config'),1);
    		}
